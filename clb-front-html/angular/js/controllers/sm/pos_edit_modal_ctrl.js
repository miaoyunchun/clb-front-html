...
app.controller('PosEditModalCtrl',function($scope, $modal, $modalInstance, $filter, $interval, $timeout, $http, shareDataService, toasterUtils){
	//posName校验规则
 ...
 // Models
	...
	$scope.originalPosCode = undefined;
	 ...
	 $scope.alerts = [
	    ...
	    var editSuccessAlert = function () {
	        $scope.alerts.push({
	            type: 'success',
	            msg : $filter('translate')('pages.sm.pos.POS_MGMT_ALERTS.POS_MGMT_ALERT_POS_EDIT_SUCCESSFUL')
	        });
	    };
	    ...
	    $scope.chkPosCodeExistence = function(inputPosCode){
	    	
	    	if($scope.fields.posCode === $scope.originalPosCode){
	    		inputPosCode.$setValidity('posCodeAvailable',true);
	    		$scope.alerts = [];
	            return;
	    	}
			 $scope.alerts = [];
			 ...
			 $http({
				 ...
				 params: {
		             ...
		             if (responseData.flag !== 'success') {
		            	 editFailAlert($filter('translate')('pages.sm.pos.POS_MGMT_ALERTS.POS_MGMT_ALERT_INVALID_POSCODE'));
		             }
		         }, function failedCallback() {
		             toasterUtils.showErrorToaster($filter('translate')('pages.common.COMMON_TOAST_TEXTS.COMMON_TOAST_TEXT_NETWORK_FAILED'));
		         }); 
			 ...
			 //edit
			 $scope.edit = function (posCode,posName,posDescription) {
			        // Clear alerts
			        $scope.alerts = [];
			        var selectedRowItems = shareDataService.common.getSelectedRowItems();
			      var selectChange=  ($scope.selectedDep.selected != selectedRowItems.depId)||($scope.selectedStatus.selected != selectedRowItems.posActive)||($scope.selectedOrg.selected!=selectedRowItems.orgId);
			        $http({
			            method: 'POST',
			            url   : CLB_FRONT_BASE_URL + 'pos/editPosInfo',
			            params: {
			            	'posId'                      	: $scope.fields.posId,
			            	'isEdited'                      : selectChange||posCode.$dirty||posName.$dirty||posDescription.$dirty,
			...
			// Close modal
			    ...
			    angular.element(function () {
			        ...
			        $scope.fields.posActive =selectedRowItems.posActive;
			       
			        
			        $scope.originalPosCode =$scope.fields.posCode;
			        
			        $scope.selectedStatus.selected = String($scope.fields.posActive);
			        $scope.selectedDep.selected = $scope.fields.depId;
...
app.controller('EditPosModalInstanceCtrl', function ($scope, $modal, $filter, toasterUtils, shareDataService) {
    $scope.showEditPosPopup = function () {
        ...
        if (table.row('.active').length === 1) {
            ...
            $modal.open({
                backdrop   : 'static',
                templateUrl: 'editPosModal.html',
                controller : 'PosEditModalCtrl',
                size       : 'lg'
            });
        } else {
