...
app.controller('ListLnPaymentModalControl', function ($scope, $modal,$timeout, $filter,toaster,lnShareDataService) {
	 $scope.listLnLending = function () {
	   var table = $('#tblLoanPaymentMgmt').DataTable();
       // If a row is selected
       if (table.row('.active').length === 1) {
           lnShareDataService.common.setSelectedRowItems(table.row('.active').data());
           if(table.row('.active').data().lncntrct_cntrct_status!='3'){
        	   $timeout(function () {
                   toaster.pop(
                       'error',
                       $filter('translate')('提示'),
        	   ...
        	   $modal.open({
                   backdrop: 'static',
                   templateUrl: 'lnPaymentMgmt.html',
                   controller: 'LnPaymentMgmtModalControl',
                   size: 'lg'
               });
           }
       } else {
