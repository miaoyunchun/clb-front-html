...
app.controller('LoanCheckMgmtCtrl', function ($scope, $http, $filter, $timeout, $interval,dataTableUtils,formUtils) {
	...
	//前端列表搜索条件（注意：查询条件需要加上param_前缀，方便后台统一处理查询条件）
	 $scope.fields = {
	        	...
	        	param_lncappf_status : undefined
		    };
	 
	 var SELECT_DOCUMENT_TYPE="933a62fc696943978b5e17cb6ac48596"; 
	 
	 var SELECT_PARAM_LNCAPPF_STATUS="66dc18613559432ba4b62bc78d508ba3";
	 
     $scope.selectedDocumentType={};
	 $scope.selectedParamLncappfStatus={};
	 $scope.documentTypeSelectOnChange= function() {
	        $scope.fields.param_lncappf_cust_type = $scope.selectedDocumentType.item.key;
	    };
	 ...
	 angular.element(function () {
		 ...
		 $http({
	            ...
	            url   : CLB_FRONT_BASE_URL + 'select/queryOptValueWithFilter',
	            params: {
	                optId: SELECT_PARAM_LNCAPPF_STATUS
	            }
	        }).then(function (response) {
	            var responseData = response.data;
...
//    $scope.placeholders = shareDataService.getPlaceHolders();
    ...
    $scope.search = function () {
        ...
        var dataTablesSelector = angular.element('#tblLoanCheck');
    ...
    $scope.$watch('$viewContentLoaded', function () {
        ...
        var tblOrganizations = angular.element('#tblLoanCheck');
        ...
        var setDataTableClickable = $interval(function () {
            ...
            if (tblOrganizations.DataTable !== undefined && $.fn.dataTable.isDataTable('#tblLoanCheck')) {
