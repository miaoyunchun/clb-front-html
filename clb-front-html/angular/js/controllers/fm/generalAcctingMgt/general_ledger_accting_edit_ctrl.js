...
app.controller('EditGeneralAcctingModalCtrl',function ($scope,$modal,$timeout,$filter,toaster,fmShareDataService) {

    $scope.showEditGeneralAcctingModal = function () {
        ...
        if (acctTable.row('.active').length === 1) {
            ...
            $modal.open({
                backdrop: 'static',
                templateUrl: 'editGeneralAcctingModal.html',
                controller: 'GeneralAcctingEditModalControl',
                size: 'lg'
            })
        }else {
            $timeout(function () {
...
app.controller('GeneralAcctingEditModalControl',function ($scope,fmShareDataService,$http,$timeout,$filter,$modalInstance) {
    ...
    $http({
        ...
        //点击提交按钮的单击事件
        $scope.editGeneralAccting = function () {
            var param = getFormData(angular.element('#formEditGeneralAcct'));
            console.log(param);
            $http({
                method:'POST',
                url:FM_URL.FM_GENERALACCTING_UPD,
                data:{'megid': 'test',
                    "user": 'test',
                    "param": param,
                    "correlid":'test'}
