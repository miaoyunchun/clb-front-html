'use strict';
//打开科目详情的modal框
app.controller('tradEditModalCtrl',function ($scope,$modal,$timeout,fmShareDataService,$http,toaster,$rootScope,$filter) {
	//权限标识
...
//	    ];
    ...
    $scope.showEditTradModal = function () {
        ...
        if (subTable.row('.active').length === 1) {
            ...
            $modal.open({
                 backdrop: 'static',
                 templateUrl: 'editTradModal.html',
                 controller: 'tradEditModalControl',
                 size: 'lg'
             });
...
app.controller('tradEditModalControl',function ($scope,$interval,$timeout,$modalInstance,$http,$rootScope,fmShareDataService,$filter) {
        ...
        if(response.status == 200){//查询成功
            ...
            if(response.data.code=="0000"){
              ...
              //根据查询结果显示24小时冲正标识
                ...
                msg=$filter('translate')('pages.fm.TRAD_LIST.ALERTS_EDIT_SUCCESS');
                $scope.alerts = [];
                editSuccessAlert();
    			...
    			msg=$filter('translate')('pages.fm.TRAD_LIST.ALERTS_EDIT_FAIL');
                $scope.alerts = [];
                editFailAlert();
  ...
  //修改按钮的单击事件
    $scope.editTradItem = function () {
        var param = getFormData(angular.element('#formEditTradingItem'));
        ...
        $http({
            ...
            if(response.status == 200){//与后台交互成功
                ...
                if(response.data.code === '0000'){//修改服务成功
                	$scope.isFormDisabled = true;
		    		msg=$filter('translate')('pages.fm.TRAD_LIST.ALERTS_EDIT_SUCCESS2');
                    $scope.alerts = [];
                    editSuccessAlert();
	    			...
	    			msg=$filter('translate')('pages.fm.TRAD_LIST.ALERTS_EDIT_FAIL2');
                    $scope.alerts = [];
                    editFailAlert();
